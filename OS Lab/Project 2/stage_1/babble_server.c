...
// semaphore for producer, consumer and mutual exclusion
    ...
    /* looping on client commands */
        while((recv_size=network_recv(newsockfd, (void**) &recv_buff)) > 0){
            ...
            if(parse_command(recv_buff, cmd) == -1){//process further cmds from registered client
                ...
                //Put command in the command_buffer              

                produce(&empty,&full,&mutexlock,&command_buffer,cmd,&input);          
            }
            free(recv_buff);
        }
...
void executor_thread_func(void *arg){
    ...
    while(1) {
        ...
        //retrieving command from buffer
        cmd = consume(&empty,&full,&mutexlock,&command_buffer,&output);
