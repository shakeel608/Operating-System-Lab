...
client_bundle_t* registration_lookup(unsigned long key)
{
    ...
    // lock to prevent writers from writing when readers are reading
    ...
    // unlocking the readlock when readers are done
...
{       
    // lock to prevent other writers from writing when one writer is writing
    ...
    // unlock for other writers to write
    if(pthread_rwlock_unlock(&rwlock) != 0){
        perror("writer thread: pthread_rwlock_wrlock error");
        return NULL;
    }    
