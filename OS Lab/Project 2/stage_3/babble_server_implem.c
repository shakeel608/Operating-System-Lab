...
static pthread_rwlock_t readwritelock = PTHREAD_RWLOCK_INITIALIZER;
...
int run_publish_command(command_t *cmd, answer_t **answer)
{
    ...
    // readWrite Lock to protect the nb_followers variable
...
{
    answer_t *the_answer=NULL;    pthread_rwlock_wrlock(&readwritelock);
